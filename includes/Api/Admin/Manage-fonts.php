...
class ASCWO_Api_Manage_fonts extends WP_REST_Controller
{
	...
	public function add_font_to_manages_fonts($request)
	{
		...
		if ($update) {
			return rest_ensure_response(
				array(
					'success' => true,
					'message' => __('Font created with success', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
	...
	public function get_manage_fonts($request)
	{
		...
		return rest_ensure_response(
			array(
				'message' => __('No Fonts Found', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
	...
	public function get_manage_font($request)
	{
		...
		return rest_ensure_response(
			array(
				'success' => false,
				'message' => __('Font not found', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
	...
	public function update_font($request)
	{
		...
		if ($all_fonts[$font_id] === $font) {
			return rest_ensure_response(
				array(
					'success' => 'same',
					'message' => __('No change observed in font', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		if ($update) {
			return rest_ensure_response(
				array(
					'success' => true,
					'message' => __('The font has been updated with success', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		return rest_ensure_response(
			array(
				'success' => false,
				'message' => __('Font update failed', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
	...
	public function delete_manage_fonts_config($request)
	{
		...
		if ($update) {
			return rest_ensure_response(
				array(
					'success' => true,
					'message' => __('The font was well removed', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		return rest_ensure_response(
			array(
				'success' => false,
				'message' => __('Deleting the font failed', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
