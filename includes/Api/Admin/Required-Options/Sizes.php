...
class ASOWP_Api_Required_Options_Sizes extends ASOWP_Api_Required_Options_Base
{
    ...
    private function normalize_size_item(array $item, int $index = 0): array
    {
        return array(
    ...
    public function add_size_item($request)
    {
        ...
        $sizes = $this->section_item_list($required_options, 'sizes');
        $sizes[] = $this->normalize_size_item($payload, count($sizes));
    ...
    public function update_size_item($request)
    {
        ...
        $payload = json_decode($request->get_body(), true);
        $payload = is_array($payload) ? $this->normalize_size_item($payload, $item_id) : array();
