	...
	public function get_theme_color_settings($request)
	{
		...
		if (!$post instanceof WP_Post) {
			return rest_ensure_response(
				array(
					'message' => __('Theme color ID invalid', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		return rest_ensure_response(
			array(
				'message' => __('No theme color settings found', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
	...
	public function update_theme_color_settings($request)
	{
		...
		if ('same' === $response) {
			return rest_ensure_response(
				array(
					'success' => 'same',
					'message' => __('No change observed in theme colors', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		if ($response) {
			return rest_ensure_response(
				array(
					'success' => true,
					'message' => __('Theme colors updated successfully', 'all-signs-customizer-for-woocommerce'),
				)
			);
		}
		...
		return rest_ensure_response(
			array(
				'success' => false,
				'message' => __('Update theme colors failed', 'all-signs-customizer-for-woocommerce'),
			)
		);
	}
