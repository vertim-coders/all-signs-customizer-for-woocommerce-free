...
class ASCWO_Product_Config
{
	...
	public function __construct($id = 0)
	{
		if ($id != 0) {
			...
			$config = get_post_meta($this->variation_id, 'product-ascwo-metas', true);

			if (isset($config[$this->variation_id])) {
				$config_id = $config[$this->variation_id];
				if (isset($config_id['config-id'])) {
					$this->settings = get_post_meta($config_id['config-id'], 'ascwo-configs-meta', true);
				}
			}
		}
	}
	 ...
	 */
	private function display_ascwo_config_on_WC_product_config($pid, $configs_ids, $title)
	{
		$meta = get_post_meta($pid, 'product-ascwo-metas', true);
		$container = "<div class='bg-black p-4'>" .
			"<h2>" . esc_html($title) . "</h2>" .
			"<div>" .
			"<select name='product-ascwo-metas[" . esc_attr($pid) . "][config-id]'>";
		...
		$container .= "</select>";
		$container .= "<input type='hidden' name='ascwo_config_nonce' value='" . wp_create_nonce('ascwo_config_nonce') . "'/>";
		$container .= "</div></div>";
	...
	public function save_config($post_id)
	{
		...
		if (isset($_POST['ascwo_config_nonce']) && wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['ascwo_config_nonce'])), 'ascwo_config_nonce') && isset($_POST[$meta_key])) {
	...
	{
		if ($column_name === 'is_ascwo_customizable') {
	...
	public function is_ascwo_customizable()
	{
		...
		if ($product->is_type('variable')) {
			...
			foreach ($variations as $variation) {
				...
				if ($ascwo_product->is_ascwo_customizable()) {
					echo wp_kses_post($ascwo_product->get_design_buttons($with_upload));
				}
			}
			...
			if ($item_id) {
				...
				if (get_option('permalink_structure')) {
					if (substr($ascwo_page_url, -1) !== '/') {
						...
						$ascwo_page_url .= "edit/$item_id/$cart_item_key/" . '?custom_qty=' . get_option($qty_key, $this->get_purchase_properties()['min_to_purchase']);
					} elseif ($order_item_id) {
						$ascwo_page_url .= "ordered-design/$item_id/$order_item_id/";
	...
	public function hide_cart_button()
	{
		global $product;
		$pid = $product->get_id();
		$configs = get_post_meta($pid, "product-ascwo-metas", true);
		if (isset($configs[$pid]['config-id'])) {
			$config_id = $configs[$pid];
			$meta_value = get_post_meta((int) $config_id['config-id'], "ascwo-configs-meta", true);
			$general_options = $meta_value["data"]["settings"]["generals"]["product"] ?? null;
			$custom_products_ids = ascwo_get_custom_products();
			...
			if (in_array($pid, $custom_products_ids) && $general_options != null && $general_options['hideAddToCartButtonOnDetailPage']) {
				wp_localize_script("ascwo-product-min", "cart_hide_button", [
					"hide_cart_button" => $general_options['hideAddToCartButtonOnDetailPage']
				]);
				add_action('wp_footer', [$this, 'ascwo_product_page_script_init']);
			}
		}
	}
	public function ascwo_product_page_script_init()
	{
		global $product;
		$pid = $product->get_id();
		...
		";
		wp_add_inline_script('ascwo-product-min', $inline_script);
		wp_add_inline_script('ascwo-tinymce-script', $inline_script);
	}
	/**
	...
	public function get_customize_btn()
	{
		...
		$ascwo_product = new ASCWO_Product_Config($product_id);
	...
	public function hide_or_display_custom_button_or_add_to_cart_button($html, $product)
	{
		...
		$configs = get_post_meta($productId, "product-ascwo-metas", true);
		$meta_value = isset($configs[$productId]['config-id']) ? get_post_meta((int) $configs[$productId]['config-id'], "ascwo-configs-meta", true) : [];
		$general_options = $meta_value["data"]["settings"]["generals"]["product"] ?? null;
	...
	public function get_button_on_single_product_summary()
	{
		...
		if (isset($configs[$pid]['config-id'])) {
			...
			//$meta_value = get_post_meta($config_id,"ascwo-configs-meta",true);
			...
			if (in_array($pid, $custom_products_ids)) {
				$ascwo_product = new ASCWO_Product_Config($product->get_id());
				if ($ascwo_product->is_ascwo_customizable()) {
					echo wp_kses_post($ascwo_product->get_design_buttons());
				}
			}
		}
	}
	...
	public function delete_product_file_when_delete_product($cart_item_key, $cart)
	{
		...
		if (!empty($cart_item_content) && isset($cart_item_content["ascwo_meta_data"]['recaps'])) {
			$meta_data = $cart_item_content["ascwo_meta_data"]['recaps'];
			$uploads = [];
				...
				if (isset($meta_data["designImages"])) {
					foreach ($meta_data["designImages"] as $key => $image) {
						...
						if (isset($path_parts["filename"], $path_parts['extension'])) {
							$file = ASCWO_IMAGE_PATH . DIRECTORY_SEPARATOR . $path_parts["filename"] . '.' . $path_parts['extension'];
							if (file_exists($file)) {
								wp_delete_file($file);
							}
						}
