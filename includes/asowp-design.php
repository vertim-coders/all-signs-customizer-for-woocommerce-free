...
class ASOWP_Design
{
	...
	public function init_hooks()
	{
		...
		add_action('wp_ajax_asowp_generate_order_zip_file', [$this, 'asowp_generate_order_zip_file_ajax']);
		add_action('wp_ajax_nopriv_asowp_generate_order_zip_file', [$this, 'asowp_generate_order_zip_file_ajax']);
				...
				<div class="omodal-dialog">
					<div class="omodal-content">
						...
						<div class="omodal-body">
							...
							<?php } else { ?>
								<div>
									<img src="<?php echo esc_url($cart_item['asowp_meta_data']["recaps"]["designImages"]["face1"][0]) ?>"
										style="width: auto; height: 500px;" />
								</div>
								<div>
									<img src="<?php echo esc_url($cart_item['asowp_meta_data']["recaps"]["designImages"]["face2"][0]) ?>"
										style="width: auto; height: 500px;" />
								</div>
							<?php } ?>
						</div>
							...
							<div>
								...
								<?php foreach ($face as $image) { ?>
									<div class="asowp-custom-options-info-infos" style="display: block !important;">
										<div>
											<p><?php echo esc_html__("file", "all-signs-options-pro") . " : " . esc_html($image["infos"]["name"]) ?>
											</p>
										</div>
										<?php if ($admin) {
							...
							<?php foreach ($recaps["designImages"] as $key => $image) { ?>
								<div style="display:flex; justify-content:center; align-items:center;">
									...
									<div style="margin:10px 0">
										<a class="button alt asowp_admin_download_image" href="<?php echo esc_url($image) ?>"
											download><?php echo esc_html__('Download File', "all-signs-options-pro") ?></a>
									</div>
								</div>
							<?php } ?>
						<?php } else { ?>
	...
	public function get_order_custom_admin_data($item_id, $item, $_product)
	{
		...
		if ($order_id && isset($order_data) && !empty($order_data)) {
			...
			if (isset($order_data["zip"])) { ?>
				<div style="margin:10px 0">
					<a class="button alt asowp_admin_download_image" href="<?php echo esc_url($order_data["zip"]) ?>"
						download><?php echo esc_html__('Download Order Zip file', "all-signs-options-pro") ?></a>
				</div> <?php
			} else {
				?>
				<div style="margin:10px 0">
					<button class="button alt asowp_admin_generate_zip_file" data-item-id="<?php echo esc_attr($item_id) ?>"
						data-nonce="<?php echo wp_create_nonce('asowp_generate_order_zip_file') ?>">
						<img src="<?php echo esc_url(ASOWP_ASSETS . '/images/im_loading.gif') ?>"
	...
	public function asowp_generate_order_zip_file_ajax()
	{
		if (wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nonce'])), 'asowp_generate_order_zip_file')) {
			if (isset($_POST['item_id'])) {
				$order_data = wc_get_order_item_meta($_POST['item_id'], 'asowp_meta_data');
				if (isset($order_data) && !empty($order_data)) {
					...
					$asowp_zip_file = $this->asowp_zip_file($order_id, $_POST['item_id'], $order_data['recaps']["output"], $order_data['recaps']['designImages'], $uploads, $order_data["recaps"]["sign"]["size"]["value"]);
					$order_data['zip'] = $asowp_zip_file;
					wc_update_order_item_meta($_POST['item_id'], "asowp_meta_data", $order_data);
					wp_send_json(array(
						'success' => true,
					));
				}
	...
	private function asowp_zip_file($order_id, $item_id, $output_settings, $previews, $uploads, $sizes)
	{
		...
		if (isset($outputOptions["zipName"]) && $outputOptions["zipName"] == true) {
			...
			if (isset($output_settings["prefix"]) && !empty($output_settings["prefix"])) {
				$zip_file = "/" . $output_settings["prefix"] . "asowp-$item_id.zip";
			} else {
				$zip_file = "/" . "asowp-$item_id.zip";
			}
		}
		...
		return ASOWP_IMAGE_URL . $zip_file;
	}
	...
	private function save_pdf_output(string $unit, array $prevImages)
	{
		...
		if (isset($prevImages["face1"])) {
			...
			foreach ($prevImages as $key => $face) {
				...
				if ($ext != 'svg') {
					require_once ASOWP_INCLUDES . DIRECTORY_SEPARATOR . 'tcpdf' . DIRECTORY_SEPARATOR . 'tcpdf.php';
					$pdf_orientation = "p";
					$pdf_margin_lr = 0;
					$pdf_margin_tb = 0;
