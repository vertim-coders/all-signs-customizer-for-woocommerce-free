...
class ASOWP_Product_Config
{
	...
	public function get_design_buttons($with_upload = false)
	{
		...
		if ($product->is_type('variable')) {
			...
			?>
			<div class="asowp-buttons-wrap-<?php echo esc_attr($product->get_type()); ?>"
	...
	public function get_templates_buttons($with_upload = false)
	{
		...
		if ($product->is_type('variable')) {
			...
			<div class="asowp-buttons-wrap-<?php echo esc_attr($product->get_type()); ?>"
				...
				$content .= '<a  href="' . $default_template_btn_url . '" class="button asowp-template-product">' . $have_pages_settings["buttons"]["productTemplateButton"] . '</a>';
			...
			if ($item_id) {
				...
				if (get_option('permalink_structure')) {
					if (substr($asowp_page_url, -1) !== '/') {
						...
						$asowp_page_url .= 'asowp-templates/' . $item_id . '/';
						if ($tpl_id) {
							$asowp_page_url .= "$tpl_id/";
						}
					}
	...
	public function get_customize_btn()
	{
		...
		if ($asowp_product->is_asowp_customizable()) {
			$configs = get_post_meta($product_id, "product-asowp-metas", true);
			$meta_templates = get_post_meta($configs[$product_id]['config-id'], 'asowp-templates', true);
			$meta_design = get_post_meta($configs[$product_id]['config-id'], 'asowp-configs-meta', true);
			...
			if (!$general_options["hideDesignButtonsOnShopPage"]) {
				...
				if ($product_class == 'WC_Product_Simple') {
					...
					if ($asowp_product->is_asowp_customizable()) {
						...
						$meta_value = get_post_meta($configs[$productId]['config-id'], 'asowp-templates', true);
