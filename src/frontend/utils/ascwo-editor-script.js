...
if (ascwo_data.page == "configurator" || ascwo_data.page == "admin") {
  fixingUrl = ascwo_configurator_data.fixing_methods_url;
  borderUrl = ascwo_configurator_data.borders_url;
}
...
function registerCustomFilters(fabricLib) {
  ...
  if (!fabricLib.Image.filters.Greenify) {
    fabricLib.Image.filters.Greenify = fabricLib.util.createClass(
      fabricLib.Image.filters.BaseFilter,
      {
        type: "Greenify",
        fragmentSource: `
  ...
  if (!fabricLib.Image.filters.Blueify) {
    fabricLib.Image.filters.Blueify = fabricLib.util.createClass(
      fabricLib.Image.filters.BaseFilter,
      {
        type: "Blueify",
        fragmentSource: `
  ...
  if (!fabricLib.Image.filters.Pinkify) {
    fabricLib.Image.filters.Pinkify = fabricLib.util.createClass(
      fabricLib.Image.filters.BaseFilter,
      {
        type: "Pinkify",
        fragmentSource: `
  ...
  if (!fabricLib.Image.filters.Orangeify) {
    fabricLib.Image.filters.Orangeify = fabricLib.util.createClass(
      fabricLib.Image.filters.BaseFilter,
      {
        type: "Orangeify",
        fragmentSource: `
...
function handleCloneCanvas() {
  // Méthode 1 : Utilisation de loadFromJSON (recommandée)
...
// Fonction utilitaire pour vérifier si le clonage a réussi
...
function handleUndo() {
    ...
    if (currentConfig.currentStateIndex == -1) {
      ...
      if (currentConfig.canvasState.length >= 1) {
        ...
        if (currentConfig.currentStateIndex != 0) {
          ...
          function recreateState(canva, stateJson) {
            ...
            currentState.objects.forEach(function (obj) {
              fabric.util.enlivenObjects([obj], function (prevObject) {
                ...
                if (
                  prevObject[0].name === "ascwo-SignText" ||
                  ...
                  console.log(prevObject[0], "ascwo-SignText");
                }
                if (prevObject[0].name === "ascwo-SignImage") {
                  prevObject[0].on("mousedown", function () {
          ...
          recreateState(
            canvas,
            currentConfig.canvasState[currentConfig.currentStateIndex - 1]
              .front,
          );
          recreateState(
            backCanvas,
  ...
  FtextObjects = handleGetTextObjects1("ascwo-SignText");
  BtextObjects = handleGetTextObjects2("ascwo-SignText");
...
function handleClearAll() {
  ...
  function recreateState(canva, stateJson) {
    ...
    currentState.objects.forEach(function (obj) {
      fabric.util.enlivenObjects([obj], function (prevObject) {
        // if(prevObject[0].name === 'ascwo-SignText'){
        //     prevObject[0].on('editing:entered', () => {
        //         handleGetAddedTextValues(prevObject[0]);
        //     });
        //     prevObject[0].on('editing:exited', () => {
...
function centerSign(canva) {
  ...
  console.log(
    ...
    `Objet: (${Math.round(finalPos.x)},${Math.round(
      finalPos.y,
    )}) | Canvas: (${Math.round(canvasCenter.x)},${Math.round(
      canvasCenter.y,
    )})`,
  );
...
function handleMiseAEchelle(rW, rH) {
  ...
  sizeRatio =
    signHPx >= signWPx / 2 && signHPx >= convertToPx(25, "cm")
      ? "big"
      : "small";
...
//     console.log(frontTextCharLength, backTextCharLength, "ascwo-SignText")
          ...
          if (obj.text.length > charsToRemove) {
            ...
            if (textType === "3D") {
              ...
              objects.forEach(function (object) {
                if (
                  object.id == obj.id &&
                  object.name == "ascwo-SignTextLayer"
                ) {
                  textLayer = object;
                }
              });
...
function handleDeleteObject(object) {
  ...
  if (textType == "3D") {
    var objects = canvas.getObjects();
    objects.forEach((obj) => {
      if (obj.name === "ascwo-SignTextLayer" && obj.id === object.id) {
        canvas.remove(obj);
      }
    });
  }
  ...
  if (target.type == "i-text" || target.type == "neon-Text") {
    ...
    removeTextById(
      object.id,
      handleGetTextObjects1("ascwo-SignText"),
      canvas.name,
    );
    removeTextById(
      object.id,
      handleGetTextObjects2("ascwo-SignText"),
      canvas.name,
    );
  ...
  if (target.name == "ascwo-SignImage") {
...
function handleCloneObject(object, imageId) {
  ...
  target.clone(function (cloned) {
    ...
    if (
      ...
      cloned.set("name", "ascwo-SignImage");
...
var activeBorder = "none";
var activeBorderColor = "";
var activeBorder2 = "none";
        ...
        if (object.name === "safeObject") {
          ...
          if (currBorder === "normal") {
            ...
            if (selectedCutline != "none") {
              ...
              // let cutline1 = handleGetObjectByName("ascwo-cutline1", canva)
              // let cutline2 = handleGetObjectByName("ascwo-cutline2", canva)
              // if(cutline1 != null){
              //   cutline1.sendToBack()
              // }
              // if(cutline2 != null){
...
async function createPreciseContourPath(size) {
  ...
  const objectsInZone = canvas
    ...
    .filter(
      (obj) =>
        obj.name == "ascwo-SignText" ||
        obj.name == "ascwo-SignImage" ||
        obj.name == "ascwo-QRCode",
    );
...
function cloneObjectForOutline(object) {
  return new Promise(async (resolve, reject) => {
    var target = object;
    var canvas = target.canvas;
    let activeFilters = target.name == "ascwo-SignImage" ? target.filters : [];
    if (target.name == "ascwo-SignImage") {
      target.filters = [];
    }
    target.clone(function (cloned) {
...
//   }
//   if (signBackground1 === "pattern" || selectedMaterialType == "advance") {
...
//       await handleSelectBorder(activeBorder2, activeBorderColor2);
//     }
//   }
...
async function handleSelectShape(
    ...
    if (safeObject != null) {
      ...
      if (newObject) {
        ...
        if (shape == "cut-to-shape") {
          ...
          targetCanvas.centerObject(newObject);
  ...
  if (signBackground2 === "pattern" && doubleFace) {
    setPattern(backCanvas, patternUrl2);
  }
  ...
  if (firstLoad) {
    ...
    if (activeSignFace === "back" && doubleFace) {
      removeBorder(backCanvas);
      await handleSelectBorder(activeBorder2, activeBorderColor2);
    }
  }
...
async function showLoader(statut) {
  let loader = document.getElementById("ascwo-loader");
  if (loader) {
    if (statut == true) {
...
function handleSetCutline(size) {
  ...
  let cutline1 = handleGetObjectByName("ascwo-cutline1");
  let cutline2 = handleGetObjectByName("ascwo-cutline2");
...
async function resetCutline(canva) {
  return new Promise((resolve, reject) => {
    if (selectedShape != "cut-to-shape") {
      let exCutline1 = handleGetObjectByName("ascwo-cutline1", canva);
      let exCutline2 = handleGetObjectByName("ascwo-cutline2", canva);
      if (exCutline1) {
                ...
                handleConvertImageToDataURI(
                  ...
                  function (dataURI) {
                    fabric.Image.fromURL(dataURI, function (img) {
                      ...
                      img.objectType = "ascwo-fixingMethods";
                      canva.add(img);
                    });
                  },
                );
...
function handleGetAddedTextValues(transform) {
  ...
  if (transform.type == "i-text" || transform.type == "neon-Text") {
    ...
    var textEditor = document.getElementById("ascwo-text-editor");
    textEditor.value = selectedText.value;
    var sizeEditor = document.getElementById("ascwo-text-size");
...
function reScaleText(textObject) {
  ...
  if (textType === "3D") {
    var objects = activeCanvas.getObjects();
    objects.forEach(function (object) {
      if (object.id == textObject.id && object.name == "ascwo-SignTextLayer") {
        if (
...
function handleChangeTextValue(event) {
  var editor = document.getElementById("ascwo-text-editor");
        ...
        objects.forEach(function (object) {
          if (
            (currentText.id == object.id &&
              object.name == "ascwo-SignTextLayer") ||
            object.name == "ascwo-SignTextLight"
          ) {
            object.set("text", String(selectedText.value));
          }
        });
...
function handleChangeTextAlign(align) {
  ...
  if (textType == "3D") {
    ...
    objects.forEach(function (object) {
      if (
        (currentText.id == object.id && object.name == "ascwo-SignTextLayer") ||
        object.name == "ascwo-SignTextLight"
      ) {
...
function handleChangeTextBorder(layer, value) {
  ...
  if (layer) {
    ...
    var objects = activeCanvas.getObjects();
    objects.forEach(function (object) {
      if (object.name == "ascwo-SignTextLayer" && object.id == currentText.id) {
...
function handleShow3dSide(state) {
  ...
  var objects = activeCanvas.getObjects();
  objects.forEach(function (object) {
    if (object.id == currentText.id && object.name === "ascwo-SignTextLayer") {
      if (state) {
...
function handleAddImageToSign(image, imageId, price) {
  ...
  var imageInput = document.getElementById("ascwo-image-input");
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        if (width > maxWidth || width < minWidth) {
          reject(
            `${__(
              "The image must be sized between",
              "all-signs-customizer-for-woocommerce-pro",
            )} ${imageSettings.fileUploadScript.uploadMinWidth}${__(
              "px and",
              "all-signs-customizer-for-woocommerce-pro",
            )} ${imageSettings.fileUploadScript.uploadMaxWidth}${__(
              "px in width",
              "all-signs-customizer-for-woocommerce-pro",
            )}`,
          );
      ...
      img.onerror = () => {
        reject(
          console.log(
            __(
              "Unable to load image for size verification.",
              "all-signs-customizer-for-woocommerce-pro",
            ),
          ),
        );
...
function handleChangeAddedSvgColor(color) {
  ...
  if (currentImage.objectType == "svg") {
    activeCanvas.getObjects().forEach((objet) => {
      if (objet.name === "ascwo-SignImage" && objet === currentImage) {
        // console.log(objet)
...
function handleMoveobject(to) {
  ...
  const relatedObjects = objects
    .filter((obj) => {
      ...
      return (
        obj.name === "ascwo-SignTextLayer" || obj.name === "ascwo-SignTextLight"
      );
    })
    .sort(
      (a, b) =>
        activeCanvas.getObjects().indexOf(a) -
        activeCanvas.getObjects().indexOf(b),
    );
  ...
  if (to === "up") {
    const topIndex = activeCanvas
      .getObjects()
      .indexOf(relatedObjects[relatedObjects.length - 1]);
...
async function handleLoadTemplateData(
  ...
  async function loadFromJSON(canva, canvasJson) {
    ...
    for (const object of canvasJson.objects) {
      if (object.name === "ascwo-SignText") {
        // console.log(object, "ascwo-SignText")
        ...
        (obj) =>
          new Promise((resolve) => {
            fabric.util.enlivenObjects([obj], async function (templateObject) {
              if (templateObject[0].name === "ascwo-SignText") {
              ...
              if (templateObject[0].name === "ascwo-SignImage") {
                templateObject[0].setControlsVisibility({
                  mt: false, // Middle top
                  mb: false, // Middle bottom
                  ml: false, // Middle left
              ...
              if (templateObject[0].name === "ascwo-QRCode") {
                templateObject[0].setControlsVisibility({
                  mt: false, // Middle top
                  mb: false, // Middle bottom
                  ml: false, // Middle left
      ...
      for (let i = textObjects.length - 1; i >= 0; i--) {
        ...
        if (obj.text.length > charsToRemove) {
          ...
          if (textType === "3D") {
            ...
            var objects = activeCanvas.getObjects();
            objects.forEach(function (object) {
              if (object.id == obj.id && object.name == "ascwo-SignTextLayer") {
                textLayer = object;
              }
            });
